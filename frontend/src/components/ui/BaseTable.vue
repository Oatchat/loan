    ...
    <table class="w-full text-[14px]">
      ...
      <tbody>
        ...
        <tr v-else-if="!rows.length">
          <td :colspan="columns.length + ($slots.actions ? 1 : 0)" class="px-5 py-14 text-center text-ink-400">
            <slot name="empty">{{ empty }}</slot>
          </td>
        </tr>
        <tr v-else v-for="(row, idx) in rows" :key="row.id ?? idx"
          ...
          :class="[row._highlight === 'overdue' ? 'border-l-[3px] border-l-danger bg-red-50/30' : 'hover:bg-brand-light/40']">
